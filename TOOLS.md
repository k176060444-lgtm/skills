...
## 🌤️ 天气推送格式规范（qweather 技能）

**技能路径**：`~/.openclaw/workspace/skills/qweather/`
**数据来源**：QWeather（和风天气）
**默认城市**：杭州（坐标 30.25, 120.15）
...
| 元素 | 颜色 | 代码 |
|------|------|------|
| 今日/明日气温 | 钴蓝加粗 | `<font color="#1E90FF"><b>` |
| 空气质量优 | 翡翠亮加粗 | `<font color="#50FA7B"><b>` |
| 空气质量良 | 普通文本 | 不加粗不加色 |
| 轻度污染 | 橙色加粗 | `<font color="#FFA500"><b>` |
| 中度污染 | 橙红加粗 | `<font color="#FF6347"><b>` |
| 重度污染 | 红色加粗 | `<font color="#FF0000"><b>` |
| 严重污染 | 深红加粗 | `<font color="#CC0000"><b>` |
| 未来3天温度 | 普通文本 | 不加粗不加色 |
...
| 任务 | 时间 | 模型 |
|------|------|------|
| 早间天气推送 | 06:50 | mimopro |
| 晚间天气推送 | 21:00 | mimopro |

### qweather 检索逻辑

- **今天推送**：today → 直接按日期字符串匹配，无需偏移
- **明天推送**：tomorrow → 直接按日期字符串匹配
- **空气质量没有明日专项 API**：只能用当前数据凑合
