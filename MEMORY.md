...
- `current_interval_usage_count` = **已用量**（窗口内已使用的请求次数）
- `remains_time` = 距离窗口滚动还剩多少毫秒
- 剩余可用 = total_count - usage_count

**示例数据（正确解读）：**
```json
{"current_interval_total_count":600, "current_interval_usage_count":546, "remains_time":3952540}
// 正确解读：总配额600，已用546，剩余54，窗口还有约3.95小时滚动重置
```
...
**错误教训：** 曾把 usage_count 当成"剩余量"，实际上是"已用量"，两者的关系是相加为 total_count。
...
### 🔧 新闻推送操作规范（必须牢记）
- **三个工具必须联合使用**：web_search 被拦截 → 立刻换 baidu-search → 再换 minimax-web-search，三个工具是并联，不是一个不行就放弃
